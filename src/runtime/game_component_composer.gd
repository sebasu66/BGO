...
func _instantiate_component(definition: Dictionary, parent: Node3D) -> void:
	...
	_log(
		...
		{"instance_id": instance_id, "component_id": component_id}
	)
	...
	if instance == null:
		_log_error(
			"COMPONENT_ROOT_INVALID", {"instance_id": instance_id, "component_id": component_id}
		)
		return
...
func _connect_component_events(instance: Node, instance_id: String, component_id: String) -> void:
	if not instance.has_signal("component_event"):
		return
	instance.connect("component_event", _on_component_event.bind(instance_id, component_id))
