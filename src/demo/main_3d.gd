...
func _begin_pointer(position: Vector2, source: String) -> void:
	...
	logger.info("POINTER_DOWN", {"source": source, "x": position.x, "y": position.y, "mode": interaction_mode})
...
func _end_pointer(position: Vector2, source: String) -> void:
	...
	logger.info("POINTER_UP", {"source": source, "x": position.x, "y": position.y, "dragged": _pointer_dragged, "distance": distance})
...
func _pick_at(screen_position: Vector2) -> void:
	...
	logger.info("TAP_NO_TARGET", {"x": screen_position.x, "y": screen_position.y, "mode": interaction_mode})
...
func _create_piece_from_state(id: String, state: Dictionary, cell: Vector2i) -> void:
	...
	pieces[id] = body
	logger.info("PIECE_CREATED", {"piece_id": id, "location_type": location_type, "position": _vec3_payload(body.position)})
...
func _update_piece_from_state(piece: Node3D, state: Dictionary, cell: Vector2i) -> void:
	...
	var state_changed := old_location_type != location_type or old_holder != holder or old_cell != cell
...
func _hand_slot_index(holder: String, piece_id: String) -> int:
	...
	for key in pieces.keys():
		...
		if piece != null and str(piece.get_meta("location_type", "board")) == "hand" and str(piece.get_meta("holder_id", "")) == holder:
			ids.append(str(key))
...
func _refresh_hand_strip() -> void:
	...
	for key in pieces.keys():
		var piece := pieces[key] as Node3D
		if piece != null and str(piece.get_meta("location_type", "board")) == "hand" and str(piece.get_meta("holder_id", "")) == player_id:
...
func _setup_logger() -> void:
	client_id = "%s-%s-%d-%d" % [client_role, player_id, Time.get_ticks_msec(), randi_range(1000, 9999)]
