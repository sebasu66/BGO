...
const BGO_GRID_BRIDGE = preload("res://src/editor/bgo_asset_placer_grid_bridge.gd")
...
func _snap_position(hit_pos: Vector3, normal: Vector3) -> Vector3:
	...
	if _presenter.options.snapping_enabled:
		...
		snapped = tangent * snapped_tangent + bitangent * snapped_bitangent + n * local_height
	# BGO's logical table grid is authoritative when the edited scene contains one.
	snapped = BGO_GRID_BRIDGE.snap_position(snapped, EditorInterface.get_edited_scene_root())
...
func _place_instance(transform: Transform3D, select_after_placement: bool):
	...
	if is_instance_valid(parent) and is_instance_valid(asset.get_resource()):
		...
		BGO_GRID_BRIDGE.annotate_placed_node(new_node, scene)
