...
func autocomplete() -> void:
	if (suggesting):
		...
		suggesting = true
		if autocomplete_provider.is_valid():
			var custom_suggestions: Variant = autocomplete_provider.call(line_edit.text)
			if custom_suggestions is Array and not custom_suggestions.is_empty():
...
var expression_handler := Callable()


func set_expression_handler(handler: Callable) -> void:
	expression_handler = handler
	...
	if not new_text.strip_edges().is_empty():
		...
		if console_commands.has(text_command):
			...
			if (arguments.size() < console_command.required):
				...
				print_error(
					"Too many arguments! Expected at most < %d >, received < %d >"
					% [console_command.arguments.size(), arguments.size()]
				)
				return
			...
			if expression_handler.is_valid() and bool(expression_handler.call(new_text)):
				return
