@tool
class_name GdssMethod_RadialGradient
extends GdssMethod
...
func _init() -> void:
	method_name = "radial_gradient"
	supported_prop_types = [GDSS.Type.COLOR]
	returns_texture = true
	parameters = [
		Param.new("inner", ParamType.COLOR),
		Param.new("outer", ParamType.COLOR),
...
func call_method(args: Array[Variant], node_id: int = -1, state_key: String = "") -> Variant:
	...
	grad.color_a = inner
	grad.color_b = outer
	grad.p0 = Vector2(0.5, 0.5)
	grad.p1 = Vector2(0.5, 1.0)
...
func interpolate_args(from_args: Array[Variant], to_args: Array[Variant], t: float) -> Array[Variant]:
	...
	return [
		...
		lerpf(_arg(from_args, 3, 1.0), _arg(to_args, 3, 1.0), t),
	]
