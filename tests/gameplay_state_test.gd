...
static func _test_validated_moves(check: Callable) -> void:
	...
	check.call(not bool(unauthorized.get("ok", false)), "other player's object is rejected")
	check.call(game.to_dictionary() == before, "ownership rejection preserves gameplay state")
	...
	check.call(game.to_dictionary() == before, "neutral denial preserves gameplay state")
	...
	check.call(game.tabletop.object_slot("neutral") == "board:c", "successful move updates occupancy")
