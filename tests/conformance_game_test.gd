...
static func run(check: Callable) -> void:
	...
	check.call(not bool(invalid.get("ok", false)), "fixture rejects an invalid out-of-turn move")
	check.call(game.to_dictionary() == before_invalid, "invalid fixture move preserves state")
	...
	check.call(game.session.result.get("winner_participant_ids", []) == ["player_1"], "fixture winner is explicit")
	check.call(
...
static func _build_game(data: Dictionary) -> GameplayState:
	...
	for index in players.size():
		...
		session.assign_participant(str(player.get("id", "")), "seat-%d" % (index + 1), "player")
	...
	for object_def in setup.get("objects", []):
		var object := LOGICAL_OBJECT_STATE.create(
			str(object_def.get("id", "")), str(object_def.get("owner_id", ""))
		)
