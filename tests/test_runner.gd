...
func _test_component_registry() -> void:
	...
	for component_id in expected:
		...
		_check(ComponentRegistry.load_scene(component_id) != null, "scene exists for %s" % component_id)

	_check(not ComponentRegistry.has_component("bgo.missing.component"), "unknown component is rejected")
	_check(ComponentRegistry.load_scene("bgo.missing.component") == null, "unknown component has no scene")

func _test_component_validation() -> void:
	var valid_board := {"columns": 8, "rows": 6, "cell_size": 1.2}
	_check(ComponentRegistry.validate_config("bgo.board.checkered", valid_board).is_empty(), "valid board config passes")

	var invalid_board := {"columns": 1, "rows": 6, "cell_size": 1.2}
	_check(not ComponentRegistry.validate_config("bgo.board.checkered", invalid_board).is_empty(), "invalid board columns are rejected")

	_check(not ComponentRegistry.validate_config("bgo.missing.component", {}).is_empty(), "unknown component config is rejected")
...
func _test_game_definition() -> void:
	...
	var missing: Dictionary = GameDefinitionLoader.load_game("res://games/does-not-exist/game.jsonh")
	_check(not bool(missing.get("ok", false)), "missing game definition fails safely")
	_check(not (missing.get("errors", []) as Array).is_empty(), "missing game definition returns a useful error")
