...
func _test_component_registry() -> void:
	...
	for component_id in expected:
		_check(COMPONENT_REGISTRY.has_component(component_id), "registry resolves %s" % component_id)
		_check(COMPONENT_REGISTRY.load_scene(component_id) != null, "scene exists for %s" % component_id)

	_check(not COMPONENT_REGISTRY.has_component("bgo.missing.component"), "unknown component is rejected")
	_check(COMPONENT_REGISTRY.load_scene("bgo.missing.component") == null, "unknown component has no scene")
...
func _test_game_definition() -> void:
	...
	var missing: Dictionary = GAME_DEFINITION_LOADER.load_game("res://games/does-not-exist/game.jsonh")
...
func _test_session_state() -> void:
	...
	_check(session.assign_participant("watcher", "seat-2", "spectator"), "spectator seat assignment succeeds")
	_check(session.assign_participant("p2", "seat-3", "player"), "second player assignment succeeds")
	...
	_check((session.result.get("winner_participant_ids", []) as Array) == ["p2"], "winner is explicit")
