...
func test_root_merges_child_property_schema() -> void:
	var root := ROOT_SCRIPT.new() as BgoGameObject3D
	var plane := PLANE_SCRIPT.new() as BgoPlaneRepresentation
	root.add_child(plane)
	root.definition_properties = {"width": 2.0, "height": 3.0}
	var schema := root.get_definition_schema()
...
func test_staggered_layout_generates_rectangular_slots() -> void:
	var root := ROOT_SCRIPT.new() as BgoGameObject3D
	add_child(root)
	var layout := LAYOUT_SCRIPT.new() as BgoSlotLayout
	root.add_child(layout)
	layout.mode = BgoSlotLayout.LayoutMode.STAGGERED_GRID
	layout.rows = 2
	layout.columns = 3
	layout.slot_size = Vector2(2.0, 1.0)
	layout.rebuild()
	var slots := layout.generated_slots()
	assert_eq(slots.size(), 6)
	assert_eq(slots[0].size, Vector2(2.0, 1.0))
	assert_ne(slots[0].position.x, slots[3].position.x)
	root.free()


func test_behavior_is_authoring_data_only_for_now() -> void:
	var behavior := BEHAVIOR_SCRIPT.new() as BgoBehavior
	behavior.rules = [
